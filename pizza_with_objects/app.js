...
pizzasInCart.addEventListener('click', (e) => {
  let deleteIndex;

  console.log(e.target);

  if (e.target.classList.contains('deletePizzaBtn')) {
    e.target.parentElement.classList.add('marked');
  }

  const pizzaTags = pizzasInCart.querySelectorAll('.pizza-tag');

  for (let i = 0; i < pizzaTags.length; ) {
    if (pizzaTags[i].classList.contains('marked')) {
      deleteIndex = i;
    }
  }

  for (const pizzaTag of pizzaTags) {
    pizzaTag.classList.remove('marked');
  }

  newPizzaOrder.deletePizza(deleteIndex);
  UI.makePizzaTag(newPizzaOrder);
});
